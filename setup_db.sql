-- You must run this SQL in your Supabase project's SQL Editor to create the required tables.
...
create index if not exists transactions_user_date on public.transactions(user_id, occurred_at desc);
...
create table if not exists public.upcoming_transactions (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  title text not null,
  amount numeric(18,2) not null,
